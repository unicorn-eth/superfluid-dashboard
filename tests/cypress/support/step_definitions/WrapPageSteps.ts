...
Then(
  /^The upgrade button is enabled and asks user to connect a wallet$/,
  () => {
...
Then(
  /^The downgrade button is enabled and asks user to connect a wallet$/,
  () => {
...
Then(
  /^The "([^"]*)" balance is shown correctly on "([^"]*)"$/,
...
Then(
  /^The underlying token balances in the wrap token dialog are shown correctly on "([^"]*)"$/,
  (network: string) => {
...
Then(
  /^The super token balances in the unwrap token dialog are shown correctly on "([^"]*)"$/,
  (network: string) => {
...
Given(
  /^Transaction dialog for ([^"]*) is shown wrapping ([^"]*) ([^"]*)$/,
  (network: string, amount: string, token: string) => {
...
Given(
  /^The transaction drawer shows a pending "([^"]*)" transaction on "([^"]*)"$/,
  (type: string, network: string) => {
    WrapPage.validatePendingTransaction(type, network);
  }
);
Given(
  /^The transaction drawer shows a succeeded "([^"]*)" transaction on "([^"]*)"$/,
  (type: string, network: string) => {
    WrapPage.validateSuccessfulTransaction(type, network);
  }
);
Given(
  /^Users super token balance of "([^"]*)" on "([^"]*)" increases by "([^"]*)" in the dashboard page$/,
  (token: string, network: string, amount: string) => {
    WrapPage.validateBalanceAfterWrapping(token, network, amount);
  }
);
Then(
  /^Wrap field input field has "([^"]*)" written in it$/,
  (amount: string) => {
    WrapPage.validateWrapFieldInputAmount(amount);
  }
);
Then(
  /^The token balances after wrapping "([^"]*)" tokens are correctly shown in the wrap page$/,
  (amount: string) => {
...
Given(
  /^User unwraps the "([^"]*)" of the selected token$/,
  (amount: string) => {
    WrapPage.clearAndInputUnwrapAmount(amount);
...
Given(
  /^Transaction dialog for ([^"]*) is shown unwrapping ([^"]*) ([^"]*)$/,
  (network: string, amount: string, token: string) => {
...
Given(
  /^Users super token balance of "([^"]*)" on "([^"]*)" decreases by "([^"]*)" in the dashboard page$/,
  (token: string, network: string, amount: string) => {
    WrapPage.validateBalanceAfterUnwrapping(token, network, amount);
  }
);
Then(
  /^Unwrap field input field has "([^"]*)" written in it$/,
  (amount: string) => {
    WrapPage.validateUnwrapInputFieldAmount(amount);
  }
);
Then(
  /^The token balances after wrapping "([^"]*)" tokens are correctly shown in the unwrap page$/,
  (amount: string) => {
...
Given(
  /^Transaction dialog for ([^"]*) is shown approving allowance of ([^"]*) ([^"]*)$/,
  (network: string, amount: string, token: string) => {
...
Given(
  /^User approves the protocol to use "([^"]*)" "([^"]*)" on "([^"]*)" if necessary$/,
...
Then(
  /^The native token "([^"]*)" balance for "([^"]*)" on "([^"]*)" is shown under the token selection button$/,
...
Then(
  /^The native token "([^"]*)" balance for "([^"]*)" on "([^"]*)" in the token list$/,
  (token: string, account: string, network: string) => {
...
Then(
  /^"([^"]*)" is selected as the token to unwrap$/,
  function (token: string) {
    WrapPage.validateTokenSelectedForUnwrapping(token);
  }
);
Then(
  /^"([^"]*)" is selected as the token to wrap and it has underlying balance of "([^"]*)"$/,
  function (token: string, balance: string) {
