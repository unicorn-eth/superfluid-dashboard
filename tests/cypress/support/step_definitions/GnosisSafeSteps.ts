...
Then(
  /^The correct wallet is connected to the gnosis app on "([^"]*)"$/,
  function (network: string) {
    GnosisSafe.validateCorrectlyConnectedWallet(network);
  }
);
Given(
  /^Gnosis safe custom app page is open on "([^"]*)"$/,
  function (network: string) {
...
Given(
  /^Manifest is loaded correctly and user accepts the warning and adds the app$/,
  function () {
