...
Given(/^User searches for "([^"]*)" as the accountable account$/, function (address:string) {
...
Given(/^User removes the selected address from the list of addresses$/, function () {
...
Given(/^User selects "([^"]*)" from the search section$/, function (address:string) {
    ExportPage.selectAddressFromSearchResults(address)
});
Given(/^"([^"]*)" is shown as one of the selected addresses for export preview$/, function (address:string) {
    ExportPage.validateSelectedAddress(address)
});
Given(/^And address search box shows that "([^"]*)" address is selected$/, function (amount:string) {
...
Given(/^User removes the selected address from the list of selected addresses$/, function () {
    ExportPage.removeAddressFromTheSelectedAddressList()
});
Given(/^User selects "([^"]*)" for the accounting export$/, function (address:string) {
    ExportPage.searchForAccount(address)
    ExportPage.selectAddressFromSearchResults(address)
    WrapPage.clickOkButton()
});
Given(/^User changes the price granularity to "([^"]*)"$/, function (period:string) {
    ExportPage.changePriceGranularityTo(period)
});
Given(/^User changes the accounting period to "([^"]*)"$/, function (period:string) {
    ExportPage.changeAccountingPeriodTo(period)
});
Then(/^Export preview is correctly fetched from the accounting API for "([^"]*)"$/, function (period:string) {
...
Then(/^Exported data for "([^"]*)" is fetched and shown correctly$/, function (type:string) {
    ExportPage.validateCorrectlyExportedData(type)
});
Given(/^User changes the export start date to "([^"]*)"$/, function (date:string) {
    ExportPage.changeExportStartDate(date)
});
Given(/^User changes the export end date with date picker to "([^"]*)"$/, function (date:string) {
...
Then(/^The "([^"]*)" column is sorted in "([^"]*)" order$/, function (column:string,ascdesc:string) {
    ExportPage.validateColumnSorting(column,ascdesc)
});
Given(/^User adds a custom filter for "([^"]*)" to "([^"]*)" "([^"]*)"$/, function (column:string,operator:string,value:string) {
    ExportPage.addCustomFilter(column,operator,value)
});
Then(/^The export preview table only shows "([^"]*)" rows with "([^"]*)"$/, function (column:string,value:string) {
...
Given(/^User searches for "([^"]*)" as an extra accountable account$/, function (address:string) {
    ExportPage.searchForExtraAccount(address)
});
Given(/^User searches for "([^"]*)" as the counterparty account$/, function (address:string) {
