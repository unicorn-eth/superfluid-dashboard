...
Feature: Stream transactional test cases
  ...
  Scenario: Creating a new stream
    ...
    Then All the details to send "1" "fDAIx" per "month" to "elvijs.eth" on "opsepolia" are set in the fields
  ...
  Scenario: Modifying a stream
    ...
    And The restore button is visible for the last transaction
    And The first row in the table shows "elvijs.eth" "receiving" an "ongoing" stream of "random" token per month since "now"
    And User restores the last transaction
