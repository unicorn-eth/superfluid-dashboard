Feature: Token page test cases
  ...
  Scenario: Token balances and net flows
    Given "Dashboard Page" is open with "ongoingStreamAccount" connected on "opsepolia"
    And User opens "opsepolia" "fDAIx" individual token page
    Then Token symbol name, icons and liquidation date in token page are shown correctly for "fDAIx" on "opsepolia"
    And "fDAIx" net flow balances are shown correctly for "ongoingStreamAccount" on "opsepolia"

  Scenario: Streams table in token page
    Given "Dashboard Page" is open with "ongoingStreamAccount" connected on "opsepolia"
    And User opens "opsepolia" "fDAIx" individual token page
    Then "opsepolia" streams are shown with the correct values in the token page
  ...
  Scenario: Distributions table in token page
    ...
    And User opens "polygon" "POLx" individual token page
