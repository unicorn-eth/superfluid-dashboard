...
Feature: Distribution transaction test cases
    ...
    Scenario: Approving a subscription
        Given Transactional account alice is connected to the dashboard on avalanche-fuji
        ...
        And Distribution approval dialog on "avalanche-fuji" shows up and user closes it
        ...
        And The transaction drawer shows a pending "Approve Index Subscription" transaction on "avalanche-fuji"
        And The restore button is not visible for the last transaction
        And The transaction drawer shows a succeeded "Approve Index Subscription" transaction on "avalanche-fuji"
    ...
    Scenario: Revoking a subscription
        ...
        And Distribution revoking dialog on "avalanche-fuji" shows up and user closes it
        And User opens the transaction drawer
        And The first distribution row in the table shows "Revoking..." pending transaction status
        And The transaction drawer shows a pending "Revoke Index Subscription" transaction on "avalanche-fuji"
        And The restore button is not visible for the last transaction
        And The transaction drawer shows a succeeded "Revoke Index Subscription" transaction on "avalanche-fuji"
        And The first distribution row in the table shows "Syncing..." pending transaction status
        And There is no pending status for the first distribution row
