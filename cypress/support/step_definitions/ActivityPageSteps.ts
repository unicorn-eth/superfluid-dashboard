...
Given(/^User changes the activity history filter to (\d+) months before$/,  (months:number) => {
    ActivityPage.changeActivityHistoryDateBack(months)
});
Given(/^Activity history data for "([^"]*)" is shown correctly on "([^"]*)"$/,  (account:string,networkType:string) => {
...
Given(/^User clicks on the "([^"]*)" toggle in the activity filter$/,  (toggle:string) => {
    ActivityPage.clickFilterToogle(toggle)
});
Then(/^No "([^"]*)" activities are shown in the activity history$/,  (type:string) => {
    ActivityPage.validateNoActivityByTypeShown(type)
});
Then(/^Activity history entries with "([^"]*)" are visible$/,  (type:string) => {
    ActivityPage.validateActivityVisibleByType(type)
});
Then(/^Only the activity history entries with "([^"]*)" are shown$/,  (address:string) => {
