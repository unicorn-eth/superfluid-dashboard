...
Then(/^"([^"]*)" streams are shown with the correct values in the token page$/,  (network:string) => {
...
Then(/^"([^"]*)" transfers for "([^"]*)" on "([^"]*)" are shown correctly$/, function (account:string,token:string,networks:string) {
    IndividualTokenPage.validateAllTokenTransfers(account,token,networks)
});
Then(/^User switches to the "([^"]*)" tab in the transfer table$/, function (tab:string) {
    IndividualTokenPage.switchTableTab(tab)
});

Then(/^Only the "([^"]*)" transfers are shown for "([^"]*)" on "([^"]*)" for "([^"]*)"$/, function (sentOrReceived:string , account:string , network:string,token:string) {
    IndividualTokenPage.validateFilteredTokenTransfers(sentOrReceived,account,network,token)
});
